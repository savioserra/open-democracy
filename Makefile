...
#   make build       build the gateway binary into ./bin/gateway
...
.PHONY: help test build run image up down logs clean tidy fmt vet

help:
	@grep -E '^[a-zA-Z_-]+:.*?##' $(MAKEFILE_LIST) | sort | awk 'BEGIN {FS = ":.*?## "}; {printf "  \033[36m%-12s\033[0m %s\n", $$1, $$2}' || true
	@echo ""
	@echo "Targets: help test build run image up down logs clean tidy fmt vet"
...
$(GATEWAY): $(shell find cmd internal chaincode -type f -name '*.go' 2>/dev/null) $(shell find internal/gateway/web -type f 2>/dev/null) go.mod go.sum
	@mkdir -p $(BIN_DIR)
